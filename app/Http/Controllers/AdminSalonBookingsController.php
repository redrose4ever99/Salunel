<?php namespace App\Http\Controllers;
	...
	class AdminSalonBookingsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "salon_bookings";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Client","name"=>"client_id","join"=>"salon_clients,first_name"];
			$this->col[] = ["label"=>"Staff","name"=>"staff_id","join"=>"staffs,title"];
			$this->col[] = ["label"=>"Service","name"=>"service_id","join"=>"salon_service,id"];
			...
			$this->form = [];
			$this->form[] = ['label'=>'Clien Id','name'=>'client_id','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'salon_clients,first_name'];
			$this->form[] = ['label'=>'Staff','name'=>'staff_id','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'staffs,title'];
			$this->form[] = ['label'=>'Service','name'=>'service_id','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'salon_service,id'];
		...
		public function getIndex() {
			//First, Add an auth
			 ...
			 $data = [];/*
			 $data['page_title'] = 'salon Booking';
			 $salon=DB::table('salons')->where('user_id',CRUDBooster::myId())->first();
			  $postdata['salon_id'] = $salon->id;
			  $staff=DB::table('staffs')->where('salon_id',$salon->id)->where('Active',1)->first();
			  $data['staffs']=DB::table('staffs')->where('salon_id',$salon->id)->where('Active',1)->orderby('id','desc')->paginate(10);
			 $data['result'] = DB::table('salon_bookings')->where('salon_id',$salon->id)->where('staff_id',$staff->id)->get();
			 */
			 //Create a view. Please use `view` method instead of view method from laravel.
			// return $this->view('salon.appoitment.index',$data);
			 return $this->view('salon.appoitment.ind',$data);
		  }
